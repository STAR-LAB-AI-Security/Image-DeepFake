...
交付的 Baseline 为 **JPEGReencodeAttack**（赛题"最小攻击方示例"：JPEG quality=85），
其余攻击作为固定池成员，供防御鲁棒性评测（test.py --stage defense）使用。

接口：
    attack(sample, quality=85, seed=42) -> {"image": ...}   # 交付 Baseline

用法：
    python attack.py                   # 对测试集伪造图施加 JPEG 隐匿，输出 data/attacked_fakes.npz
...
class JPEGReencodeAttack:
    """JPEG 重编码攻击（交付 Baseline，对应赛题 A0-1 与"最小攻击方示例"）。
...
class CompositeHiddenAttack:
    ...
    def __init__(self, steps: int = 3, seed: int = 7, quality: int = 75) -> None:
        ...
        self._rng = np.random.RandomState(seed)
        self._pool = [
            ("jpeg", lambda s: JPEGReencodeAttack(
...
def _jpeg_attack_with_random(quality: int = 75, random_quality: bool = False, seed: int = 0):
    ...
    return JPEGReencodeAttack(quality=q, seed=seed)
...
if __name__ == "__main__":
    ...
    atk = JPEGReencodeAttack(quality=args.quality, seed=args.seed)
