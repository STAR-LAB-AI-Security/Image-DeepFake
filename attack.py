...
class JPEGReencodeAttack:
    ...
    def apply(self, image: np.ndarray) -> np.ndarray:
        ...
        步骤：
          1. 保证输入为 RGB uint8 H×W×3
          2. 通过 BytesIO 编码为 JPEG（给定 quality），再解码回 ndarray
          3. 尺寸/dtype 与输入保持一致

        为什么有效：
          - 生成模型/伪造图像常在频域留下异常高频能量、周期性网格或
            方块边界；JPEG 量化表对这些高频系数做粗量化，能显著削弱
            这些痕迹。
          - 只做一次重压缩即可，避免多次压缩引入新的块效应。
        """
        # 1) 规范输入
        ...
        assert image.ndim == 3 and image.shape[2] == 3, \
            f"expect HxWx3 RGB, got {image.shape}"

        # 2) 编解码 JPEG（用 PIL，若后端未提供 to_pil 也可用 cv2）
        try:
            ...
            Image.fromarray(image, mode="RGB").save(
                ...
                subsampling=0,      # 4:4:4，保留色度，减少额外失真
                optimize=False,
            )
            buf.seek(0)
...
def attack(sample, quality=85, seed=42):
    """模块级入口，委托到 JPEGReencodeAttack。"""
    return JPEGReencodeAttack(quality=quality, seed=seed).apply(sample)
