...
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""攻击方法：频域洗白（Frequency Laundering / FFT 低通）

原理：
    DeepFake 检测器的核心特征（FFT 高频带、SRM 残差、8×8 DCT 高频系数）
    都在衡量"高频能量"。伪造图的融合边界振铃与内部高频噪声让高频能量异常
    偏高。本攻击用 FFT Sigmoid 软低通直接抑制高频分量，检测器的高频特征
    被大幅削弱，fake 置信度跌破阈值。低频（人脸轮廓/肤色主体）几乎不变，
    视觉保真度与 PSNR 保持得很好。

接口（平台契约）：
    attack(sample) -> {"image": RGB uint8 H×W×3，与输入同尺寸}
    build_attack(name, **params) -> 攻击实例
"""
...
class JPEGReencodeAttack:
    """频域洗白攻击（FFT Sigmoid 软低通）。

    注意：类名沿用 baseline 的 JPEGReencodeAttack 以保持兼容，
    但实现的是频域洗白（name="frequency_laundering"）。
    """

    name = "frequency_laundering"

    def __init__(self, quality=85, seed=42, cutoff=0.30, strength=0.5, **kwargs):
        # 兼容 platform 可能传入的额外参数
        self.quality = int(quality)
        self.seed = int(seed)
        self.cutoff = float(kwargs.get("cutoff", cutoff))
        self.strength = float(kwargs.get("strength", strength))
...
def build_attack(name: str, **params):
    """按名称构造攻击实例（供 test.py / 平台使用）。"""
    table = {
        "jpeg": JPEGReencodeAttack,
        "frequency_laundering": JPEGReencodeAttack,
    }
...
def build_attack_pool(pool=None):
    """构造固定攻击池（供防御方评测使用）。"""
    return [("jpeg", JPEGReencodeAttack(quality=85, seed=42))]
